 ...
 * perform better than unnecessary long one.
 */
public class SlowPerformanceExample {
  public static long testBankAccount(BankAccount bankAccount) throws Exception {
    ...
    // 5 threads will deposit money in shared bank account
    Depositor[] depositors = new Depositor[5];
    for (int i = 0; i < depositors.length; i++) {
      depositors[i] = new Depositor(bankAccount, 1000);
      depositors[i].start();
    }
...
/**
 * Банкова сметка, която "печати" фактура след извършване на депозит.
 * Тя работи няколко пъти по бързо (на моето ПС с този вход - около 6пъти) 
 * от бавната банкова сметка. Причината е, че бавния метод printInvoice е 
