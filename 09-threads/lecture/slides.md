...
- многозадачни операционни системи. Процеси. Комуникация между процеси (IPC)
...
- Нишката не може да бъде стартирана повторно (при повторно стартиране ще се хвърли `IllegalThreadStateException`)
...
- Класове в [`java.util.concurrent.atomic`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/atomic/package-summary.html) пакета
...
atomicInt.set(2025);
System.out.println(atomicInt.get()); // 2025
```
...
System.out.println(atomicInt.incrementAndGet()); // 2026

// thread-safe вариант на i += x (i=i+x)
System.out.println(atomicInt.addAndGet(5)); // 2031
...
- Събужда една нишка, чакаща за съответния монитор (ако има такава)
- Извикването на `notify()` винаги става в критична секция по обекта, който ползваме за монитор
- `notify()` не освобождава монитора - той ще се освободи едва при напускане на критичната секция

---

### `notify()` vs. `notifyAll()`

- `notify()` – събужда една произволна нишка, чакаща за този монитор (ако има такава). Полезно е само когато сме сигурни, че само една нишка може да продължи изпълнението си, и не искаме да "платим" цената да събудим всички
