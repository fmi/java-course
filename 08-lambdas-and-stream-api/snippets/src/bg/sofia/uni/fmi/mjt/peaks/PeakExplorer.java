...
public class PeakExplorer {
	 ...
	 */
	public String getEarliestAscentedPeaks(int n) {
		return peaks.stream().filter(p -> p.firstAscent() > 0).sorted(Comparator.comparing(Peak::firstAscent)).limit(n).map(Peak::name).collect(
				Collectors.joining(", "));
	}

	/**
	 * Определете върхът с най-голяма изпъкналост изкачен през конкретна година
	 */
