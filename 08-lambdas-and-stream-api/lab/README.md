# Amazon Trends :chart_with_upwards_trend: :package:
...
който по даден низ, представляващ ред от dataset-a, връща `Order` обект. Всеки `Order` се идентифицира еднозначно от своя идентификатор (`id`).

👉 Подсказка: Датите в dataset-a са във формат dd-MM-yy (напр. 14-03-25). За парсването им ще ви влезе в употреба [`java.time.format.DateTimeFormatter.ofPattern(...)`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/time/format/DateTimeFormatter.html).

### Категории продукти

Продуктите в поръчките спадат към една от няколко категории, описани от следния enum в пакета bg.sofia.uni.fmi.mjt.orders.domain:
...
Начините на плащане се моделират чрез следния enum в пакета bg.sofia.uni.fmi.mjt.orders.domain:
...
Текущото състояние на поръчката се описва от следния enum в пакета bg.sofia.uni.fmi.mjt.orders.domain:
...
В пакета bg.sofia.uni.fmi.mjt.orders.loader създайте клас `OrderLoader`, който имплементира логиката за зареждане на данните. Вашата задача е да довършите имплементацията на метода `load`:

```java
package bg.sofia.uni.fmt.mjt.orders.loader;

import bg.sofia.uni.fmt.mjt.orders.domain.Order;
...
В пакета `bg.sofia.uni.fmi.mjt.orders.analyzer` създайте клас `OrderAnalyzerImpl`, който има публичен конструктор `OrderAnalyzerImpl(List<Order> orders)` и имплементира интерфейса `OrderAnalyzer`:

```java
package bg.sofia.uni.fmi.mjt.orders.analyzer;

import bg.sofia.uni.fmi.mjt.orders.domain.Category;
import bg.sofia.uni.fmi.mjt.orders.domain.Order;
import bg.sofia.uni.fmi.mjt.orders.domain.PaymentMethod;
import bg.sofia.uni.fmi.mjt.orders.domain.Status;
...
public interface OrderAnalyzer {
    ...
    /**
     * Returns an immutable list of all orders.
     *
     ...
     * @throws IllegalArgumentException if customer is null, empty, or blank
     */
    ...
    /**
     * Returns the top N most frequently ordered products.
    ...
    /**
     * Computes the total revenue for each {@link Category}.
     ...
     * </ul>
     *
     * @return a list of suspicious customer names
     */
     ...
     * If multiple locations tie, sort them alphabetically.
     *
...
- **Дати**: Форматът на датата в CSV файла е `dd-MM-yy` (напр. `14-03-25`). Стандартният `LocalDate.parse()` очаква `yyyy-MM-dd`. Ще ви е необходим `DateTimeFormatter`.
