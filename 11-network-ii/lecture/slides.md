...
TCP установява връзка чрез т.нар. *three-way handshake*
(3 съобщения, 2 участника)

- `SYN`  “I want to connect and here’s my sequence number”

- `SYN-ACK`  “I agree, here’s mine”

- `ACK` “Confirmed, we’re synchronized”

<br/>

- Всеки край избира случаен Initial Sequence Number (ISN)
- Sequence numbers са 32-битови цели числа
- Те номерират байтовете в TCP потока

`ACK = N` означава:  *„Получих всички байтове до N−1, очаквам байт N“*

---

### TCP: Full-Duplex комуникация

- TCP е двупосочен байтов поток
- Данни текат едновременно в двете посоки
- И клиентът, и сървърът изпращат ACK-ове
- ACK-овете често се "piggyback-ват" върху сегменти с данни
...
- Състоянието на буфера се определя от четири атрибута:
    - *капацитет* (capacity): максималния брой елементи, които може да съдържа буферът. Задава се при създаването му и не може да се променя
