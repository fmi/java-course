...
- **Заявка** (*Request*) – инициатор е клиентът – подава информация на сървъра, достъп до кой ресурс иска да получи и каква операция иска да извърши с него (и евентуални входни параметри). Клиент (условно наречен *User-Agent* в HTTP) може да бъде всяко софтуерно приложение, спазващо правилата на протокола на комуникация
- **Отговор** (*Response*) – изпраща се от уеб сървъра, като резултат от изпълнението на клиентска заявка. Уеб сървърът е приложение, което приема заявки през Интернет (обикновено от браузър) и връща уеб съдържание – страници, изображения, данни и др. То работи по правилата на HTTP и доставя ресурсите, които потребителят е поискал.
...
- *Хедъри* - Възможно е да дефинираме множество хедъри, като всеки от тях заема точно един ред и следва формата: “Име на хедър: Стойност на хедър”
...
Служат за "временни" кодове, т.е. заявката е пристигнала, но сървърът все още не е готов с резултата. Не дават индикация, дали заявката е била успешна или не. На практика почти винаги клиентските библиотеки ги скриват. 
...
Content-Length: 8582
Last-Modified: Sat, 15 Nov 2025 12:45:26 GMT
```
...
- 34% от всички web sites (HTTP/3: 37%)
...
- Част от JDK (от Java 11 насам)
- Използва се за изпращане на HTTP заявки и получаване на отговори
- Поддържа HTTP/1.1 и HTTP/2
- Работи както синхронно, така и асинхронно
- Основният пакет: `java.net.http`

---

### `HttpClient` – нашият „браузър“ в Java
...
- Java HTTP Client-ът по подразбиране работи по HTTP/2
- Ако сървърът не поддържа HTTP/2, заявката автоматично се "downgrade"-ва до HTTP/1.1
